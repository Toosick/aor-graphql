...
describe('buildGqlQuery', () => {
    ...
    it('returns the correct query for GET_LIST', () => {
        expect(buildGqlQuery(introspectionResults)(resource, GET_LIST, queryType, params)).toEqual(
    ...
    it('returns the correct query for GET_MANY', () => {
        expect(buildGqlQuery(introspectionResults)(resource, GET_MANY, queryType, params)).toEqual(
            'query allCommand($foo:Int!){items:allCommand(foo:$foo){foo,linked{foo},resource{id}},total:_allCommandMeta(foo:$foo){count}}',
        );
    });
    it('returns the correct query for GET_MANY_REFERENCE', () => {
        expect(buildGqlQuery(introspectionResults)(resource, GET_MANY_REFERENCE, queryType, params)).toEqual(
            'query allCommand($foo:Int!){items:allCommand(foo:$foo){foo,linked{foo},resource{id}},total:_allCommandMeta(foo:$foo){count}}',
        );
    });
    it('returns the correct query for GET_ONE', () => {
        expect(
            buildGqlQuery(introspectionResults)(resource, GET_ONE, { ...queryType, name: 'getCommand' }, params),
